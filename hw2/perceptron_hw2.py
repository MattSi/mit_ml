...
def gen_flipped_lin_separable(num_points=20, pflip=0.25, th=np.array([[3],[4]]), th_0=np.array([[0]]), dim=2):
    '''
    Generate difficult (usually not linearly separable) data set by
    ...
    def flip_generator(num_points=20):
        X, y = gen_lin_separable(num_points=20, dim=dim, th=th, th_0=th_0)
...
def plot_separator(ax, th, th_0):
    ...
    if abs(th[1, 0]) > eps:
        pts += [np.array([x, (-float(th_0) - x * th[0, 0]) / th[1, 0]])  for x in (xmin, xmax)]
    if abs(th[0, 0]) > 1.0e-6:
        pts += [np.array([(-float(th_0) - y * th[1, 0]) / th[0, 0], y])  for y in (ymin, ymax)]
    in_pts = []
    for p in pts:
...
def super_simple_separable_through_origin():
    '''
    Return d = 2 by n = 4 data matrix and 1 x n label matrix
    '''
    X = np.array([[2, 3, 9, 12],
                  [5, 1, 6, 5]])
    y = np.array([[1, -1, 1, -1]])
...
def super_simple_separable():
    '''
    Return d = x by n = 4 data matrix and 1 x n label matrix
    '''
    X = np.array([[2, 3, 9, 12],
                  [5, 2, 6, 5]])
    y = np.array([[1, -1, 1, -1]])
...
def xor_more():
    '''
    Return d = 2 by n = 8 data matrix and 1 x n label matrix
    '''
...
def test_eval_learning_alg(eval_learning_alg, perceptron):
    ...
    eval_learning_alg method for evaluating learning algorithm perceptron your perceptron learning algorithm method
    '''
...
def perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        for i in range(col):
            x_i = data[:,i:i+1]
            y_i = labels[:,i:i+1]
            if y_i * positive(theta, x_i, theta_0) <=0.0:
                theta += y_i * x_i
                theta_0 += y_i
            if hook:
                hook((theta, theta_0))
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    return ths/(col*T), th_0s/(col*T)

def eval_classifier(learner, data_train, labels_train, data_test, labels_test):

    (th, th0) = learner(data_train, labels_train)

    mark = score(data_test, labels_test, th, th0)
    return mark / data_test.shape[1]
...
def xval_learning_alg(learner, data, labels, k):
    (row, col) = data.shape
    data_folds = np.array_split(data, k, axis=1)
    label_folds = np.array_split(labels, k, axis=1)

    acc_sum = 0.0
    for i in range(k):

        data_test = data_folds[i]
        label_test = label_folds[i]

        data_train = np.concatenate(data_folds[:i] + data_folds[i+1:], axis=1)
        labels_train = np.concatenate(label_folds[:i] + label_folds[i+1:], axis=1)

        acc = eval_classifier(learner, data_train, labels_train, data_test, label_test)
        acc_sum += acc
...
if __name__ == '__main__':
    ...
    #print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 100))
    #print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 100))

    #print(eval_learning_alg2(perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 100))
    #print(eval_learning_alg2(averaged_perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 100))
