...
def feature_transformation_1c():
    data = np.array([[200, 800, 200, 800],
                     [0.2, 0.2, 0.8, 0.8]
                     ])
    labels = np.array([[-1, -1, 1, 1]])

    perceptron_origin(data, labels)
    return data, labels

def test_polynomial_aaa(raw_features):
    d, n = raw_features.shape
    result = []  # list of column vectors
    for j in range(n):
        features = []
...
if __name__ == '__main__':
    print(test_polynomial_aaa(np.array([[1, 2, 3]])))

    #feature_transformation_1c()
